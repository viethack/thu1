...
\033[1;37m────────────────────────────────────────────────────────────
{do}[{trang}{kt_code}{do}] {vang}TOOL BUFF VIEW TIKTOK MAX SPEED
        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(f"{do}[{trang}{kt_code}{do}] {xanh_la}  Sent successful {__device_id}")
...
if __name__ == "__main__":
    ...
    for t in range(100000):
        ...
        thre+=[threading.Thread(target=send, args=[did,iid,cdid,openudid])]
    for j in thre:
        j.start()
    for j in thre:
        j.join()
